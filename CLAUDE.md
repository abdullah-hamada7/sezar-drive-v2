...
This document defines the behavioral, architectural, and procedural rules governing Claude when participating in the structured multi-role software development lifecycle for the Fleet Transportation & Trip Management Platform.
...
Claude operates as a **role-bound agent** within a controlled workflow.

Claude must only act in the role explicitly assigned in the prompt.
...
* Admin-provisioned driver accounts with `must_change_password` flag
* Face verification at every shift start (rekognition:CompareFaces)
* QR-based vehicle validation and QR-based vehicle search
* Configurable inspection policy (4-direction photos vs checklist)
* Strict trip and shift state machines
* Expense logging with optional approval
* Daily revenue aggregation (resets daily)
* PDF and Excel reporting
* Asynchronous iTrack GPS integration
* Immutable audit logging
* Damage reporting workflow (auto-locks vehicle)
* Vehicle locking and maintenance state
* Passenger baggage tracking (Number of Bags)
...
* Passenger list MUST include "Number of Bags" for each passenger.
...
* PendingVerification (Includes sub-states: IdentityCheck, VehicleScan, FacialMatch)
* Active
* Closed
...
* Face verification must achieve a match score > 80% for activation (Manual Review for lower scores).
...
Claude must ensure outputs consider:
...
* Top Right Toast Notifications for all errors (auto-dismiss 5s)
* No automatic dialing (text-only display for phone numbers)
...
* Inspection workflow
* Expense UI
* Identity comparison view (Uploaded vs Details)
* Admin Verification Filters (Status, Name)
* Report export UI
...
Claude must explicitly design for:
